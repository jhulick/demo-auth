...
describe('Basics - ngModules:', function() {
    ...
    tCore
        .value('config', {appTitle: 'Mock Customers', apiBase: '/api/mma'})
        .factory('dataservice', function(config, voice) {
            return {
                getCustomers: function() {
                    voice.say('Core got Customers from ' + config.apiBase + '/customers');
                    return [
                        {name: 'Hulk'},
                        {name: 'Thor'},
                        {name: 'Black Widow'}
    ...
    tFeature1
        .factory('customer', function(voice) {
            return {
                customer: function() {
                    voice.shout();
                }
    ...
    describe('when untouched', function() {
        ...
        it('shouts when avenging', inject(function(customer) {
            $log.reset();
            customer.customer();
    ...
    describe('when test.block2.slogan changed', function() {
        ...
        it('shouts different slogan when avenging', inject(function(customer) {
            $log.reset();
            customer.customer();
            var log = $log.log.logs[0];
